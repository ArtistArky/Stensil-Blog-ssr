...
const Card20: FC<Card11Props> = ({
  ...
  const { setcurrentPost } = useGlobalContext();
  ...
  return (
    ...
    >
      ...
      <Link to={{pathname: postHref}} onClick={() => setcurrentPost(currentPost)}  className="absolute inset-0"></Link>
      ...
      <div className="p-4 flex flex-col flex-grow space-y-3">
          ...
          <h2 className="nc-card-title block text-[18px] text-base font-semibold text-neutral-900 dark:text-neutral-100 ">
            <Link to={{pathname: postHref}} onClick={() => setcurrentPost(currentPost)} className="line-clamp-2" title={title}>
              {title}
            </Link>
          </h2>
